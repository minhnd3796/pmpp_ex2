...
__global__ void conv_h_gpu_smem_kernel(unsigned int *dst_data, unsigned int *src_data, float *kernel_data, int width, int height, int kernel_size, size_t pitch)
{
...
__global__ void conv_v_gpu_smem_kernel(unsigned int *dst_data, unsigned int *src_data, float *kernel_data, int width, int height, int kernel_size, size_t pitch)
{
    extern __shared__ unsigned int shared_src_data[];
    int tx = blockIdx.x * blockDim.x + threadIdx.x;
    int ty = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void conv_h_gpu_cmem_kernel(unsigned int *dst_data, unsigned int *src_data, int width, int height, int kernel_size, size_t pitch)
{
    int tx = blockIdx.x * blockDim.x + threadIdx.x;
    if (tx >= width)
        return;
...
__global__ void conv_v_gpu_cmem_kernel(unsigned int *dst_data, unsigned int *src_data, int width, int height, int kernel_size, size_t pitch)
{
    int tx = blockIdx.x * blockDim.x + threadIdx.x;
    if (tx >= width)
        return;
